...
std::string WAAIOperations::callFuntion(std::string keyword, std::vector<std::string> words)
{
	...
	case 1:
	{
		...
		{
			return numberToString(Add(doubleNumbers));
		}
		return numberToString(Add(intNumbers));
	}
	case 2:
	{
		if (doubleNumbers.size() != 0)
		{
			return numberToString(Subtract(doubleNumbers));
		}
		return numberToString(Subtract(intNumbers));
	}
	case 3:
	{
		if (doubleNumbers.size() != 0)
		{
			return numberToString(Multiply(doubleNumbers));
		}
		return numberToString(Multiply(intNumbers));
	}
	case 4:
	{
		if (doubleNumbers.size() != 0)
		{
			return numberToString(Divide(doubleNumbers));
		}
		return numberToString(Divide(intNumbers));
	}
	case 5:
	...
	for (size_t i = 0; i < keywords.size(); i++)
	{
		...
		case 1:
		{
			...
			if (hasDecimal(number1) || hasDecimal(number2))
			{
				...
				words.erase(words.begin() + keywords[i] - 1);
				words[keywords[i] - 1] = numberToString(temp);
